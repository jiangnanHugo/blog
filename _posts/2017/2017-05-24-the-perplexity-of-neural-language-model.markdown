...
where $h_t$ is the hidden state output at time step $t$, and $\hat{y}\in R^{|V|}$ is a probability distribution over the vocabulary, same cross entropy loss function but predicting words instead of classes:
